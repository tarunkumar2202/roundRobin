...
{                    // process structure
	...
	int completetime;
	int roundbacktime;
}f[100], s[100], m[100];        // Queues for Faculty and Students.......

int n, fc=0, sc=0, mc=0;        // global variables
int quanta;

void input();           // for taking inputs from user...
void prmapping();       // for scheduling according to priority..
void roundrobin();      //  round_robin algorithm implmentation
void display();         //   for displaying results....
...
void prmapping()        // comparing and scheduing jobs in queue......
{
...
void roundrobin()        // Applying Round-Robin Algorithm....
{
...
void display()      // function that prints results to screen.....
{
...
{
	printf("\n\n\t\t\t\t\t\t ONLINE QUERY HANDLER");
 	printf("\n\t\t\t\t\t\t ******************** \t\t\n ");
	printf("\n**>Enter time in 2400 hours format. For example for 10:30 am enter 1030"
